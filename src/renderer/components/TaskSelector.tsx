...
import { Plus, AlertCircle, Clock, Clock9, Square, Check } from 'lucide-react';
...
export default function TaskSelector({ onCreateTask, disabled = false }: TaskSelectorProps) {
  ...
  const sortedTasks = useMemo(() => {
    ...
    return activeTasks.sort((a, b) => getPriorityScore(b) - getPriorityScore(a));
  }, [tasks]);
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">
            {showAllTasks ? '全部任务（按优先级排序）' : '推荐任务（紧急优先）'}
          </h3>
